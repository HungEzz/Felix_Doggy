...
export const Bones: React.FC = () => {
  ...
  const brands = ['BarkBites', 'Kibble Kraft', 'RawFeast', 'NutriPaw'];
  const allProducts = useSelector((state: RootState) => state.products.items);

  const maxPrice = useMemo(() => {
    const categoryProducts = allProducts.filter(p => p.category === 'food');
    if (categoryProducts.length === 0) return 100;
    return Math.ceil(Math.max(...categoryProducts.map(p => p.price)));
  }, [allProducts]);

  const filteredProducts = useMemo(() => {
    let result = allProducts.filter(p => p.category === 'food');
    if (searchQuery.trim()) {
      ...
      <main className="max-w-[1440px] mx-auto px-6 pt-2 pb-8 md:pt-4 md:pb-12">
        ...
        <nav aria-label="Breadcrumb" className="mb-6">
          <ol className="flex items-center space-x-2 text-text-secondary font-mono text-xs list-none p-0 m-0">
            ...
            <li aria-current="page" className="font-black text-accent uppercase">Dog Food</li>
          </ol>
        </nav>
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="font-display text-4xl md:text-5xl text-accent mb-2 transform -rotate-1 origin-bottom-left uppercase font-black">Dog Food</h1>
